...
func (c AliyunCloud) NewAudioFile(fileLink string) (string , *sdk.Client , error) {
	...
	fmt.Println("识别查询结果：", postResponseContent)
	//校验请求
	...
	return "" , client , errors.New("err code"+statusText + ": 录音文件识别请求失败 !")
}
...
func (c AliyunCloud) GetAudioFileResult(taskId string , client *sdk.Client , callback func(result []byte)) error {
	...
	fmt.Println(DOMAIN, API_VERSION, PRODUCT, GET_REQUEST_ACTION, taskId)
	...
	if statusText != STATUS_SUCCESS {
		return errors.New("err code"+statusText + ": 录音文件识别失败 !")
	}
